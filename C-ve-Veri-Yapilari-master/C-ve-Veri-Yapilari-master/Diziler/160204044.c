...
int skor=10;
int acilan_alan=0; //Acilan alanla ilgili uyari verirken linux ortaminda sıkıntılar cikiyordu bu yontemle cozdum.

void skor_guncelle(patlayici* a);
void ilklendir(patlayici d[][BOYUT]);
void hamle_yap(int satir, int sutun, patlayici d[][BOYUT]);
void komsulari_tara(int satir, int sutun, patlayici d[][BOYUT]);  /*prototip tanimlari*/
void goster(patlayici d[][BOYUT]);
int tamamlandi_mi(patlayici d[][BOYUT]); //yazarken karisiklik olmamasi icin fonksiyonlarda dizi ismini hep "d" olarak adlandirdim.
...
int main()
{
    ...
    do{
        ...
        if(acilan_alan==1)
        {
           printf("ACILMIS ALAN! FARKLI BIR ALAN DENEYIN\n"); acilan_alan=0;
        }
...
void ilklendir(patlayici d[][BOYUT])
{
    ...
    i=0;
    srand(time(NULL));
    int ary[BOYUT+1][BOYUT+1]={0}; //ilk degerleri 0 olarak atadik.
    int x,y;
